...
!
!---------------------------------------------------------------------------------------
! Fortran 90 Module 'DUMMY_UNITS'
! Copyright (C) 2002  Grant W. Petty
!
! Written by Grant W. Petty and Michael A. Walters
!    with suggestions by Howard W. Ludwig
!
! This module is indended for use with the module 'FAKEUNITS'.  See FAKEUNITS for more
! information.
!--------------------------------------------------------------------------------------
...
! Define base units
  TYPE(preal), PARAMETER :: u_meter    = preal(1.0e+0) 
  TYPE(preal), PARAMETER :: u_kilogram = preal(1.0e+0) 
  TYPE(preal), PARAMETER :: u_second   = preal(1.0e+0) 
  TYPE(preal), PARAMETER :: u_kelvin   = preal(1.0e+0) 
  TYPE(preal), PARAMETER :: u_ampere   = preal(1.0e+0) 
  TYPE(preal), PARAMETER :: u_mole     = preal(1.0e+0) 
  TYPE(preal), PARAMETER :: u_candela  = preal(1.0e+0)
  TYPE(preal), PARAMETER :: u_radian   = preal(1.0e+0) 
...
! want to keep track of radians (and steradians), then replace the above declaration with
!
